...
import { ArrowRight, Mail, Lock, Compass, Smartphone, ArrowLeft, KeyRound } from "lucide-react";
...
function LoginForm() {
  ...
  const isMockMode =
    ...
    (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_URL.includes('wyqgznxflwlapbipmcal'));
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) {
      setErrorMsg("Email address is required for OTP login");
      return;
    }
    ...
    if (isMockMode) {
      setTimeout(() => {
        setOtpSent(true);
        setSuccessMsg("Mock OTP sent! Enter any 6-digit code to log in.");
        setIsLoadingState(false);
      }, 1000);
      return;
    }
      ...
      <AnimatePresence mode="wait">
        {authMethod === 'password' ? (
          <motion.form 
            ...
            onSubmit={(e) => {
              ...
              startTransition(async () => {
                await login(data);
                // Redirect is handled inside actions.ts server redirects, but in case of failure:
                setIsLoadingState(false);
              });
            }}
          >
            ...
            <div className="flex items-center justify-between text-sm font-medium">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-gray-200 accent-organic-black" />
                <span className="text-gray-500">Remember me</span>
              </label>
              <Link href="#" className="text-organic-black hover:underline font-bold">Forgot Password?</Link>
            </div>
            ...
            >
              {isPending ? "Logging in..." : "Sign In"} <ArrowRight className="w-5 h-5 ml-2" />
            </button>
          </motion.form>
        ) : (
          <motion.div 
          ...
          >
            ...
            >
              <ArrowLeft className="w-4 h-4" /> Back to Password Sign In
            </button>
            ...
            {!otpSent ? (
              <form onSubmit={handleSendOTP} className="space-y-5">
                <div className="text-center p-4 bg-gray-50 rounded-[1.5rem] border border-gray-100">
                  ...
                  <h3 className="font-bold text-organic-black">Passwordless OTP Sign In</h3>
                  <p className="text-xs text-gray-500 font-medium">We will send a 6-digit login code to your email inbox.</p>
                </div>
              ...
              <form onSubmit={handleVerifyOTP} className="space-y-5">
                <div className="text-center p-4 bg-gray-50 rounded-[1.5rem] border border-gray-100">
                  ...
                  <h3 className="font-bold text-organic-black">Verify OTP Code</h3>
  ...
  return (
    <div className="flex items-center justify-center min-h-screen px-4 pt-20 pb-12 relative overflow-hidden bg-organic-light dark:bg-organic-green-dark">
      ...
      >
        ...
        <h2 className="mb-2 text-3xl font-black text-center text-organic-black dark:text-white">Welcome Back</h2>
        <p className="mb-8 text-center text-gray-500 font-medium">Enter your details to access your travel OS.</p>
        ...
        <p className="mt-8 text-center text-sm text-gray-500 font-medium">
          New to Traverse? <Link href="/auth/signup" className="text-organic-black dark:text-white hover:underline font-bold">Create an account</Link>
        </p>
      </motion.div>
    </div>
  );
